...
OUTPUT_BASE = "../assets/full_features_v4"
...
def train_scm(X, y, parents_dict, train_idx, test_idx, output_dir):
    ...
    print(f"[SCM] {output_dir.split('/')[-1]} SCM Metrics:")
...
if __name__ == "__main__":
    ...
    # --- Plotting model performances based on PRIMARY_METRIC ---
    ...
    for dag_key in ["DAG_2_Infrastructure_Mediator", "DAG_3_Flood_Driven"]:
        ...
        if os.path.exists(scm_path):
            with open(scm_path) as f:
                scm_metrics = json.load(f)
                if PRIMARY_METRIC in scm_metrics:
                    scores.append((f"SCM_{dag_key.split('_')[1]}", scm_metrics[PRIMARY_METRIC]))
