...
ASSETS_DIR_MODEL: str = "../assets/full_features/scm_dag_1_independent/models"
...
def _load_assets():
    model = joblib.load(os.path.join(ASSETS_DIR_MODEL, "Property_Damage_GT.joblib"))
    scaler = joblib.load(os.path.join(ASSETS_DIR_MODEL, "scaler.joblib"))
    with open(os.path.join(ASSETS_DIR_MODEL, "Property_Damage_GT_model_config.json")) as f:
        cfg = json.load(f)
...
if __name__ == "__main__":
    ...
    for idx in test_indices:
        try:
            # Get scaled row as a pandas Series for easy reordering
            original_row = df.iloc[[idx]][scaler.feature_names_in_]
            original_scaled_array = scaler.transform(original_row)[0]

            # Convert to Series with feature names from scaler
            scaled_series = pd.Series(original_scaled_array, index=scaler.feature_names_in_)

            # Reorder based on model input order
            ordered_scaled = scaled_series[X_cols].values  # now matches model expectation

            # Predict
            original_pred = model.predict([ordered_scaled])[0]
            ...
            for desired_class in {0, 1, 2} - {original_pred}:
                try:
                    ...
                    # Step 1: Select and reorder features as expected by the scaler
                    cf_row_for_scaler = cf_row[scaler.feature_names_in_]

                    # Step 2: Apply scaling
                    cf_scaled_array = scaler.transform(cf_row_for_scaler)[0]

                    # Step 3: Convert to Series and reorder to match model input
                    cf_scaled_series = pd.Series(cf_scaled_array, index=scaler.feature_names_in_)
                    cf_scaled_ordered = cf_scaled_series[X_cols].values

                    # Step 4: Predict
                    cf_pred = model.predict([cf_scaled_ordered])[0]
